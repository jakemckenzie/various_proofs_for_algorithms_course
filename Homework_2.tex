...
\Procedure{Max}{$a,b$}
    return (a $<$ b) ? b : a
\EndProcedure
For what it is worth, I don't think this algorithm will find the max element if it is contained within the first third of the array(we never iterate a), but it is the self-reduction stated.
\end{algorithmic}
\end{algorithm}
\end{enumerate}
\end{document}